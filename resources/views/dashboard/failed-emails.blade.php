                    ...
                    <table class="table m-0" id="failed-mails-table">
                        ...
                        <tbody>
                            @foreach ( $failedMails as $failedMail )
                                <tr data-id="{{ $failedMail->id  }}" data-pretty-sender-name="{{ $failedMail->from() }}">
                                    <td>
                                        <a data-value="{{ $failedMail->timestamp }}" href="{{ $failedMail->detailViewUrl() }}" target="_blank">
                                            {{ (new \Carbon($failedMail->timestamp))->format('m/d/Y h:i A') }}
                                        </a>
                                    </td>
                                    <td>
                                        <?php $failedMessage = empty($failedMail->delivery_status_message) ? $failedMail->delivery_status_description : $failedMail->delivery_status_message; ?>
                                        <button type="button" class="btn btn-sm {{ $failedMail->severity == 'temporary' ? 'btn-warning' : 'btn-danger'}}"
                                            data-toggle="popover"
                                    ...
                                    <td>
                                        {{ $failedMail->message_from }}
                                    </td>
                                    <td>
                                        {{ $failedMail->recipient }}
                                    </td>
                                    <td>
                                        <?php $linkedWebsite = $failedMail->linkedWebsite(); ?>
                                        @if( is_null($linkedWebsite) )
                                            <a class="website-sender-value">
                                                Find Website
                                            </a>
                                        @else
                                            <a href="//{{ getCleanUrl($linkedWebsite->website) }}/webadmin" target="_blank">
                                                {{ $linkedWebsite->name }}
                                            </a>
                                        @endif
                                    </td>
